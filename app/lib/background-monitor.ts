...
export interface BackgroundMonitorPlugin {
  ...
  saveFirebaseConfig(options: { apiKey: string; projectId: string; userId: string; authToken: string; phone: string; fcmToken: string; serverKey: string; clientEmail: string; privateKey: string }): Promise<{ saved: boolean }>;
